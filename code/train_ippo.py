...
# ==========================================
def train():
    parser = argparse.ArgumentParser(description="VMAS IPPO Implementation")
    ...
    print(f"IPPO Running on device: {device}")
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(args.num_steps):
            ...
            obs_buf[step], don_buf[step] = next_obs, next_done

            with torch.no_grad():
                act, logp, _, val = agent.get_action_and_value(next_obs.view(-1, obs_dim))
        ...
        # GAE
        with torch.no_grad():
            nxt_val = agent.get_value(next_obs.view(-1, obs_dim)).view(args.num_envs, n_agents)
        ...
        for _ in range(args.update_epochs):
            ...
            for start in range(0, b_obs.shape[0], b_obs.shape[0] // args.num_minibatches):
                ...
                _, newlogp, ent, newval = agent.get_action_and_value(b_obs[mb_inds], b_act[mb_inds])
    ...
    np.save("rewards_ippo.npy", np.array(all_episode_rewards))
    torch.save(agent.state_dict(), "ippo_transport_params.pth")
    print("IPPO Training finished. Data saved to rewards_ippo.npy")
